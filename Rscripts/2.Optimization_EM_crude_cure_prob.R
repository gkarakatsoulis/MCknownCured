...
# ---------------------------
# ----------------------------------------------------------------------
# Derive the function for the EM algorithm (crude cure probability)
# ----------------------------------------------------------------------
# ---------------------------
...
em = function(mydf, incidence_vars, latency_vars, cure_id_vars, tol = 10^-4){
  ...
  while(difference > tol & iter < 100){
    ...
    if (iter == 1){ # Initialization of Weights and Parameters
      ...
      cure_id_prob = (sum(mydf$true_cure_status == 0 & mydf$censor_status == 1)) / (nrow(mydf) - sum(mydf$w_i))
      
      params = c(beta, beta_latency, cure_id_prob)
      
      mydf$w_i = ifelse(!is.na(mydf$observed_cure_status),
                        mydf$observed_cure_status,
                        mydf$predicted_inc * mydf$predicted_lat / ((1-mydf$predicted_inc)*(1-cure_id_prob) + mydf$predicted_inc * mydf$predicted_lat))
    ...
    params_updated = c(beta, beta_latency, cure_id_prob)
