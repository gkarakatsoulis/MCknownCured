...
# ---------------------------
# ----------------------------------------------------------------------
# Derive the function for the EM algorithm (cured T infinite)
# ----------------------------------------------------------------------
# ---------------------------

em = function(mydf, incidence_vars, latency_vars, tol = 10^-4){
  ...
  while(difference > tol & iter < 100){
    ...
    if (iter == 1){ # Initialization of Weights and Parameters
      ...
      params = c(beta, beta_latency)
      
      mydf$w_i = ifelse(mydf$censor_status == 1,
                        1,
                        mydf$predicted_inc * mydf$predicted_lat / ((1-mydf$predicted_inc)+ mydf$predicted_inc * mydf$predicted_lat))
    ...
    params_updated = c(beta, beta_latency)
...
tmp = lapply(df, function(mydf){
  ...
  mydf$observed_time = ifelse(mydf$censor_status == 1 & mydf$true_cure_status == 0,
                              mydf$observed_time[which.max(mydf$observed_time)] + 1,
                              mydf$observed_time)
  
  mydf$censor_status = ifelse(mydf$censor_status == 1 & mydf$true_cure_status == 0,
                              0,
                              mydf$censor_status)
  
  
  
  result = em(mydf, incidence_vars, latency_vars)
  ...
  ## Derive the subsets
  mydf_uncured = subset(mydf, censor_status == 1)
  ...
  while (i <= nboots) {
    ...
    bootparams[[i]] = em(bootdata, incidence_vars, latency_vars)
