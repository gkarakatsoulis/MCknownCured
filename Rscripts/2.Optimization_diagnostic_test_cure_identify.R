...
em = function(mydf, incidence_vars, latency_vars, cure_id_vars, tol = 10^-4){
  ...
  while(difference > tol & iter < 100){
    ...
    if (iter == 1){ # Initialization of Weights and Parameters
      ...
      mydf$uncured_known = mydf$censor_status == 1 & mydf$true_cure_status == 1
      
      mod_cure_id = glm(censor_status ~ cure_id_mat, data = mydf, subset = !uncured_known, family = 'binomial')
      
      beta_cure_id = mod_cure_id$coefficients
      names(beta_cure_id)[-1] = paste0('cure_id_mat_', cure_id_vars)
      names(beta_cure_id) = gsub('\\(Intercept\\)', 'cure_id_mat_intercept', names(beta_cure_id))
      
      
      mydf$predicted_cure_id = predict(mod_cure_id, newdata = mydf, type = 'response')
      
      params = c(beta, beta_latency, beta_cure_id)
      
      mydf$w_i = ifelse(!is.na(mydf$observed_cure_status),
                        mydf$observed_cure_status,
                        mydf$predicted_inc * mydf$predicted_lat / ((1-mydf$predicted_inc)*(1-mydf$predicted_cure_id) + mydf$predicted_inc * mydf$predicted_lat))
    ...
    ## Cure identification part
    mod_cure_id = glm(censor_status ~ cure_id_mat,
                      data = mydf,
                      subset = !uncured_known,
                      weights = 1-w_i,
                      family = 'binomial')
